...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if config["split"].get("use_model_val") or config["split"].get("use_test"):
        raise ValueError("P3A forbids model validation and test text calls")
    if config["extractor"].get("external_knowledge_allowed") is not False:
        raise ValueError("P3A requires external_knowledge_allowed=false")
    return config
...
def build_extractor(config: dict[str, Any]) -> EventExtractor:
    ...
    return EventExtractor(
        ...
        ledger_path=ROOT / "results/v6/p3a/api_budget_ledger.json",
