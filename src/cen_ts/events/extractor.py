...
from .client import LLMRequestError, LLMResponse, OpenAICompatibleClient
from .schemas import EVENT_SCHEMA_JSON, EVENT_SCHEMA_VERSION, EventExtractionResult, EventRecord, TextVariantRecord


PROMPT_VERSION = "p_extract_v1"
EVENT_EXTRACTION_PROMPT = """You are a strict information extraction engine. Extract material events from only the provided source text.

Rules:
1. Use only the provided source text. Do not use external knowledge or common-knowledge completion.
2. Do not add people, organizations, places, quantities, dates, or events absent from the source.
3. A source may contain multiple events. Return at most the five most important events.
4. Every event must have a non-empty evidence_span copied verbatim from the source text.
5. Distinguish report_time from event_time_start and event_time_end.
6. Explicit future events are factuality=forecast. Forecasts, opinions, and hypotheticals must not be observed.
7. If there is no explicit material event, return no_event=true and events=[].
8. If target linkage is not explicit, use affected_target=null and expected_direction=unknown.
9. Do not invent lag values. Use null lag fields when the text does not specify them.
10. Use intensity=unknown unless strength is explicit.
11. event_type_raw must be concise and grounded in the text; event_type_canonical may be null.
12. Output strict JSON matching the schema, with no Markdown or commentary.
13. Give each event an extraction_confidence from 0 to 1 reflecting extraction certainty.
...
class EventExtractor:
    """Grounded event extractor with strict schema, budget, and cache controls."""

    def __init__(
        self,
        api_config: APIConfig | None = None,
        *,
        prompt_template: str = EVENT_EXTRACTION_PROMPT,
        ...
        if api_config is None:
            return
        if not cache_directory or not ledger_path:
            raise ValueError("cache_directory and ledger_path are required")
        self.prompt_template = prompt_template
        self.schema_text = EVENT_SCHEMA_JSON
    ...
    def transform(self, source_text: str, timestamp: str, context: dict[str, Any]) -> TextVariantRecord:
        if self.api_config is None:
            raise RuntimeError("Event extraction requires API configuration")
    ...
    def extract(
        ...
        *,
        ...
        if self.api_config is None:
            raise RuntimeError("API configuration is not initialized")
