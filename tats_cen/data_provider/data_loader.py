...
def _series_sha256(series):
    return _sha256_bytes(series.astype("string").fillna("<NA>").str.cat(sep="\n").encode("utf-8"))


def _frame_sha256(df):
    return _sha256_bytes(df.to_csv(index=False, lineterminator="\n").encode("utf-8"))
...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        df_raw = pd.read_csv(os.path.join(self.root_path,
                                          self.data_path))
        original_df = df_raw.copy(deep=True)
        ...
        cols.remove('date')
        text_name = getattr(self.args, "text_column", None) or "fact"
        if text_name not in original_df.columns:
            raise ValueError(f"Required text column {text_name!r} is missing from {self.data_path}. No fallback to fact is allowed.")
        if len(original_df[text_name]) != len(original_df[self.target]):
            raise ValueError("Text row count must equal numeric row count.")
        ...
        missing_mask = pd.isnull(self.text.reshape(-1))
        ...
        text_series = original_df[text_name].iloc[border1:border2]
        text_lengths = text_series.astype("string").fillna("").str.len()
        self.text_manifest = {
            "flag": self.flag,
            "data_path": self.data_path,
            "text_mode": getattr(self.args, "text_mode", "raw"),
            "text_column": text_name,
            "row_count": int(len(text_series)),
            "numeric_row_count": int(len(self.data_x)),
            "text_row_count_matches_numeric": int(len(text_series)) == int(len(self.data_x)),
            "text_sha256": _series_sha256(text_series),
            "non_empty_rate": float(text_series.astype("string").fillna("").str.len().gt(0).mean()),
            "unique_rate": float(text_series.astype("string").fillna("<NA>").nunique(dropna=False) / max(len(text_series), 1)),
            "length_min": int(text_lengths.min()) if len(text_lengths) else 0,
            "length_mean": float(text_lengths.mean()) if len(text_lengths) else 0.0,
            "length_max": int(text_lengths.max()) if len(text_lengths) else 0,
            "missing_count": missing_count,
            "split_border1": int(border1),
            "split_border2": int(border2),
            "window_count": int(self.tot_len) if hasattr(self, "tot_len") else int(len(self.data_x) - self.seq_len - self.pred_len + 1),
            "date_sha256": _series_sha256(original_df["date"].iloc[border1:border2]),
            "target_sha256": _series_sha256(original_df[self.target].iloc[border1:border2]),
            "non_text_identity_sha256": _frame_sha256(original_df.drop(columns=[text_name])),
        }
        if getattr(self.args, "run_dir", None):
            _write_json(Path(self.args.run_dir) / f"text_manifest_{self.flag}.json", self.text_manifest)
